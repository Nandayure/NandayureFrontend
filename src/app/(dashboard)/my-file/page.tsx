...
const PdfFileGridUser = () => {
  ...
  const fetchFiles = async (): Promise<File[]> => {
    ...
    const response = await fetch(
      `${process.env.NEXT_PUBLIC_BACKEND_URL}/google-drive-files/MyFiles`,
      {
        headers: {
  ...
  const {
    ...
    queryKey: ['files'],
    queryFn: fetchFiles,
    enabled: Boolean(token), // Solo ejecuta la consulta si EmployeeId y token están disponibles
